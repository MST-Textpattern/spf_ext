...
p. REQUIRES: Texpattern 4.4.1 and PHP 5 and shell access (for sym-linking existing files).

p. *Please read the instructions and notes below before use.*

p. This plugin creates external files you can manage and edit via the admin interface. All files are saved to the directory you create in step 1 of the instructions below. Syntax highlighting is available via the "Codemirror admin theme":https://github.com/spiffin/CodeMirrorTextpattern (currently HTML, PHP and JavaScript - more to come).

p. "DOWNLOAD":https://raw.github.com/spiffin/spf_ext/master/spf_ext_v0.1.txt
